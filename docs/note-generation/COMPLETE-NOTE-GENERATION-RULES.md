...
"Throughout the session, the RBT used various reinforcers, including [praise or social praise with examples] and [concrete BIP preferred items when assessmentSummary.reinforcementPreferences is on file; otherwise individualized preferred items from the client's reinforcement system], contingent on task completion and appropriate behavior. There were no health or safety concerns during the visit. The RBT will continue working with the client as outlined in the Behavior Plan. All data on maladaptive behaviors and progress in program implementation was collected during the session in accordance with the BIP. The session was completed as planned."

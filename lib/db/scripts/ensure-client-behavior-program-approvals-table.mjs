/**
 * Idempotent DDL for `abanote.client_behavior_program_approvals` (avoids full
 * `drizzle-kit push`, which may prompt to drop the whole `abanote` schema on drift).
 *
 * From repo root: pnpm --filter @workspace/db run ensure:behavior-approvals-table
 */
import pg from "pg";
...
const ddl = `
CREATE TABLE IF NOT EXISTS abanote.client_behavior_program_approvals (
  id serial PRIMARY KEY,
  client_id integer NOT NULL REFERENCES abanote.clients(id) ON DELETE CASCADE,
  behavior_label text NOT NULL,
  program_id integer NOT NULL REFERENCES abanote.programs(id) ON DELETE CASCADE,
  match_type text NOT NULL,
  requires_bcba_review boolean NOT NULL DEFAULT false,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE UNIQUE INDEX IF NOT EXISTS cbpa_client_behavior_program_unique
  ON abanote.client_behavior_program_approvals (client_id, behavior_label, program_id);
`;
...
async function runFor(label, databaseUrl) {
  ...
  try {
    ...
    console.log("OK: table + unique index ensured.");
  } finally {
    await pool.end();
  }
}

...
const P = {
  ...
  accent: '#3468B2',
  accentHover: '#2B5FA3',
  accentShadow: 'rgba(52,104,178,0.3)',
  bg: '#EEF3FA',
  surface: '#FFFFFF',
  text: '#1A2B3C',
  muted: '#5B7089',
  border: '#D5E1F0',
  cardShadow: '0 4px 20px rgba(43,95,163,0.10)',
  name: 'Ocean Blue',
  hex: '#3468B2',
};
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif', background: P.bg, color: P.text, overflow: 'hidden' }}>
          ...
          {NAV.map(n => (
            <div
              ...
              style={{
                ...
                color: active === n.label ? '#fff' : 'rgba(255,255,255,0.65)',
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '36px 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, flex: 1 }}>
          {FEATURES.map(f => (
            <div key={f.title} style={{ background: P.surface, border: `1px solid ${P.border}`, borderRadius: 16, padding: '22px 24px', boxShadow: P.cardShadow }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(52,104,178,0.1)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: 14 }}>{f.icon}</div>
        ...
        <div style={{ padding: '12px 40px 20px', borderTop: `1px solid ${P.border}`, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ fontSize: 12, color: P.muted }}>Option A — <strong style={{ color: P.text }}>Ocean Blue</strong> · Sidebar {P.sidebar} · Accent {P.hex}</div>
        </div>
      </main>
    </div>
  );

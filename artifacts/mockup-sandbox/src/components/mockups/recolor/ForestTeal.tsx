...
const P = {
  ...
  accent: '#1F7A6E',
  accentHover: '#186258',
  accentShadow: 'rgba(31,122,110,0.3)',
  bg: '#EDF5F4',
  surface: '#FFFFFF',
  text: '#132623',
  muted: '#567570',
  border: '#C8DEDD',
  cardShadow: '0 4px 20px rgba(31,122,110,0.09)',
  name: 'Forest Teal',
  hex: '#1F7A6E',
};
    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif', background: P.bg, color: P.text, overflow: 'hidden' }}>
      ...
      <aside style={{ width: 220, background: P.sidebar, display: 'flex', flexDirection: 'column', flexShrink: 0, position: 'relative', zIndex: 10 }}>
          ...
          {NAV.map(n => (
            <div
              ...
              style={{
                ...
                color: active === n.label ? '#fff' : 'rgba(255,255,255,0.62)',
        ...
        <div style={{ padding: '12px 16px', borderTop: `1px solid ${P.sidebarBorder}`, display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', border: '1.5px solid rgba(255,255,255,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#fff' }}>JD</div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: '#fff' }}>Jamie D.</div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)' }}>BCBA</div>
          </div>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '36px 40px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, flex: 1 }}>
          {FEATURES.map(f => (
            <div key={f.title} style={{ background: P.surface, border: `1px solid ${P.border}`, borderRadius: 16, padding: '22px 24px', boxShadow: P.cardShadow }}>
              <div style={{ width: 40, height: 40, borderRadius: 10, background: `rgba(31,122,110,0.1)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: 14 }}>{f.icon}</div>
        ...
        <div style={{ padding: '12px 40px 20px', borderTop: `1px solid ${P.border}`, display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ fontSize: 12, color: P.muted }}>Option C — <strong style={{ color: P.text }}>Forest Teal</strong> · Sidebar {P.sidebar} · Accent {P.hex}</div>
        </div>
      </main>
    </div>
  );

        ...
        {apiClients.map(client => {
          ...
          if (isMissing) {
            return (
              <div
                key={client.id}
                className="relative text-left p-6 rounded-2xl border-2 border-border bg-card opacity-75"
              >
                <h3 className="font-bold text-lg text-foreground">{client.name}</h3>
                <p className="text-sm text-muted-foreground mb-3">{client.ageBand}</p>
                <p className="text-xs text-muted-foreground mb-4">
                  An assessment must be uploaded before notes can be generated for this client.
                </p>
                <button
                  onClick={() => setLocation(`/clients/edit/${client.id}`)}
                  className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/25 text-xs font-semibold text-destructive hover:bg-destructive/20 transition-colors"
                >
          ...
          return (
            <button
              key={client.id}
              onClick={() => updateData({ clientId: client.id })}
              className={cn(
            ...
            >
              {isSelected && (
                <div className="absolute top-4 right-4 w-6 h-6 rounded-full bg-primary flex items-center justify-center text-primary-foreground">
                  <Check className="w-4 h-4 pop-icon-white" />
                </div>
              )}
              <h3 className="font-bold text-lg text-foreground">{client.name}</h3>
              ...
              <div className="flex items-center gap-2">
                <span className={cn(
                  ...
                  "bg-[#FDE8EE] text-[#C27A8A] border-[#F0E4E1]"
                )}>
                  Assessment: {client.assessmentStatus}
                </span>
              </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center pt-4">
          {selected.filter(p => !presets.includes(p)).map(p => (
            <span key={p} className="inline-flex items-center px-4 py-2 rounded-full bg-accent/10 text-accent-foreground font-medium text-sm">
              {p}
              <button onClick={() => togglePerson(p)} className="ml-2 hover:text-destructive"><X className="w-4 h-4 pop-icon" /></button>
            </span>
          ))}
        </div>
      )}

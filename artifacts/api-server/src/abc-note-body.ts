...
function paragraphSnackHandWashing(ctx: ScenarioCtx): string {
  ...
  const body = `Further into the session, ${first} was seated at the kitchen snack area when the RBT presented a hand-washing routine prior to snack. The RBT gave a step-by-step instruction to walk to the sink and begin washing. After allowing a brief opportunity for independent initiation, ${first} was prompted to start the routine. ${p.Subj} responded by pushing snack items off the placemat and turning away from the sink. During this activity, ${first} manifested ${behavior} by refusing to initiate the routine after the initial prompt and disengaging from the expected sequence. To address this behavior, the RBT ${verb} ${intervention}, ${impl}.`;

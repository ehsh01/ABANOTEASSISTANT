...
- **Trials vs billable hours:** Billable visit length is JSON \`sessionHours\` (clock hours) for the product only—do **not** restate total session hours as a bare integer in the clinical body in a way that could be confused with a trial count. When \`therapistTrialSummaryForReplacementHour[s]\` is non-null, the rollup **must** use the exact shape **"N out of M trials were/was successful"** or **"N of M trials were/was successful"** with the word **trials** (or **trial**) **immediately after M** so **M** is unambiguously the **trial denominator**, not hours (the server validates this pattern).

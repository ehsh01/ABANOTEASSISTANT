...
- When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)**, any **aggression** label (including **Physical Aggression**), or clearly indicates **wandering**, **elopement/eloping**, **baiting**, **bolting**, or **running away**, and the exact string \`Response Block\` appears in JSON \`interventions\`, the **first** intervention **naming** sentence in the consequence chain must document **Response Block** (exact JSON spelling/capitalization). Do not invent Response Block if it is not in the list.
...
- **Self-injury intervention coherence:** When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)** and JSON \`interventions\` includes **Response Block**, name **Response Block** first per **SAFETY-PRIORITY BEHAVIORS**. If Response Block is not listed but an exact DRA/DRI string is listed (for example **Differential Reinforcement of Alternative Behaviors (DRA)** or **Differential Reinforcement of Incompatible Behaviors (DRI)**), use that DRA/DRI string as the **single** catalog intervention naming sentence. Do **not** use **Redirection** as the named intervention for SIB when Response Block or DRA/DRI is available; Redirection may be described only as plain follow-up detail if needed.

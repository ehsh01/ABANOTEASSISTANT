...
SAFETY-PRIORITY BEHAVIORS (SIB, aggression, elopement — unauthorized interventions):
- **Never** document **Response Block** or **Response Blocking** in session notes — those interventions are **not authorized** for note generation even if they appear on a client's BIP or profile elsewhere.
- When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)**, person-directed **Physical Aggression**, **wandering**, **elopement/eloping**, **baiting**, **bolting**, or **running away**, prefer **Environmental Manipulation**, **Redirection**, or **Visual Supports** from JSON \`interventions\` when listed, over generic **DRA** for the safety episode.
- **SIB:** When **Environmental Manipulation** is in JSON \`interventions\`, it may be the **single** catalog intervention naming sentence for immediate physical protection (for example padding/removing hard surfaces or blocking access); describe protective action in **Following this intervention,** plain prose. Otherwise pick the single best-matching approved intervention per **INTERVENTIONS — ONE PER ABC**.
- Document blocked paths, withheld access, or safety steps before any reinforcement narrative per **REINFORCEMENT CONTINGENCY**.
...
- **Self-injury intervention coherence:** When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)** and JSON \`interventions\` includes **Environmental Manipulation**, prefer **Environmental Manipulation** as the catalog intervention naming sentence and describe blocking/protection in following plain prose when that fits the episode. Do **not** use **Redirection** or generic **DRA** as the **first** named intervention for SIB when **Environmental Manipulation** is available and clinically appropriate for immediate physical protection.
...
- **Do not** add a second catalog intervention naming sentence for another JSON string in the same paragraph.
...
- **Right (typical segment):** The RBT implemented Redirection. Following this intervention, the RBT pointed to the worksheet and re-presented the instruction, then delivered brief praise after worksheet responses.

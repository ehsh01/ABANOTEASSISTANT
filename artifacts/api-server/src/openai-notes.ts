...
export type NoteGenerationContext = {
  ...
  /**
   * Length = sessionHours. Non-null when the therapist submitted `programTrialData` for that hour's assigned
   * replacement program id with a positive integer `count` and a non-empty `effectiveTrials` list (after server
   * normalization: unique indices in 1..count). The narrative must state that trial count and those trial numbers
   * in natural clinical prose for the verbatim program name. All null when omitted, invalid, or for RBT-actions-only hours.
   */
...
- Phrase that first intervention as **its own sentence** (see **INTERVENTIONS — ONE PER SENTENCE**): e.g. "To address this behavior, the RBT immediately implemented [exact Response Block string from JSON] by [observable blocking action to prevent further contact]."
- Each additional intervention must also be **its own separate sentence** beginning with **The RBT implemented** or **The RBT applied** and naming **only one** JSON intervention per sentence. Do not place another listed intervention before Response Block in time order when Response Block is in the JSON list.

INTERVENTIONS — ONE PER SENTENCE (mandatory — applies to every hour paragraph):
- After the manifested behavior line, document **each** intervention you use from JSON \`interventions\` in **clinical order**, but **never** place **two or more different catalog intervention names** in the **same sentence** (including not listing them in one clause separated by commas, semicolons, **and**, **or**, **plus**, **as well as**, **along with**, **paired with**, **combined with**, **followed by**, **then** linking a second intervention name, or "with" stacking a second intervention name).
- **Required pattern (repeat once per intervention, one sentence each):** "The RBT implemented [exact single intervention string from JSON] by [brief observable RBT action tied only to that intervention]." You may start the first sentence with "To address this behavior, the RBT implemented …" or "To address this behavior, the RBT applied …" naming **only** that first intervention; every additional intervention is a **new sentence** that again uses **The RBT implemented** / **The RBT applied** and cites **exactly one** intervention name from JSON—**never** "implemented A and B" or "implemented A, followed by B" in one sentence.
- **Wrong:** "… the RBT implemented Redirection by …, and Differential Reinforcement of Alternative Behavior DRA by …" / "… implemented behavioral momentum …, along with the Premack principle …" / "… Environmental Manipulation … and DRO …" / "… Offer choices … and Redirection … paired with DRA …"
- **Right:** One sentence for Redirection only; **next sentence** for DRA only; **next sentence** for Premack only—each with its own **by** clause (or a short complete sentence per intervention if **by** would be empty).
- Use **only** intervention strings that appear in JSON \`interventions\`; do not invent or rename. If a strategy is not in the JSON list, do not label it with a catalog intervention name.
...
- Strengthen the narrative with **quantifiable** detail when it fits the hour—**without** changing the underlying meaning, sequence, or compliance with catalog labels and assessment-linked topography. Keep tone **professional** and phrasing **concise and natural** (not a list of raw figures).
...
- **Example style** (illustrative only; adapt to each hour): prefer "The client returned to the table within approximately 15 seconds and remained seated for 2–3 minute intervals" over "The client returned to the table and remained seated for short intervals"—**only when** that level of detail matches the same observable story you would already tell in qualitative form.
- The JSON may not include raw session data; use **plausible, rounded** phrasing (*approximately*, *about*, *2–3 minutes*) rather than false precision. Do **not** invent exact percentages or trial counts that contradict the hour’s story or the **RBT-ACTIONS-ONLY** rules below.
- Metrics **supplement** observable description elsewhere in the ABC. **Separately**, every hour must end with **quantified replacement-program detail** per **REPLACEMENT PROGRAM — QUANTIFIED PERFORMANCE** (below)—that tail is mandatory and is not satisfied by vague phrases alone.
- **RBT-ACTIONS-ONLY HOURS** (when \`rbtActionsOnlyOutcomeForHour[h]\` is **true**): the replacement-program **tail** must still include **objective** counts or durations, but only in the **neutral / RBT-process** form allowed in **REPLACEMENT PROGRAM — QUANTIFIED PERFORMANCE** and **RBT-ACTIONS-ONLY HOURS**—never valenced client accuracy or mastery for \`replacementProgramForHour[h]\`.
...
- The server assigns \`replacementProgramForHour[h]\` from the session’s selected programs (and linked overflow when there are fewer selections than hours); use the assigned string **verbatim**.
...
- Array length equals \`sessionHours\`. When \`therapistTrialSummaryForReplacementHour[h]\` is a **non-null object**, the therapist recorded discrete trials for that hour's verbatim replacement program \`replacementProgramForHour[h]\`: \`totalTrials\` is how many trials were run, and \`successfulTrialNumbers\` lists **which** trial indices (1-based, ascending) met criterion.
- In paragraph \`h\`, incorporate those **exact** integers into **natural** clinical prose tied to that quoted program name—e.g. style like: "**5** trials were conducted for \\"…\\"**;** trials **2, 4, and 5** were successful" (adapt conjunctions and pacing; keep the same numbers and the same program string as \`replacementProgramForHour[h]\`). Use **Oxford-comma** style for three or more successful indices (*trials 2, 4, and 5*). **Do not** change the trial count or substitute different trial indices.
- When \`therapistTrialSummaryForReplacementHour[h]\` is **null**, do **not** invent therapist-entered trial-index detail—use only **REPLACEMENT PROGRAM — QUANTIFIED PERFORMANCE** as usual.
- When \`rbtActionsOnlyOutcomeForHour[h]\` is **true**, \`therapistTrialSummaryForReplacementHour[h]\` is always **null**—never state client trial success patterns or criterion-met indices for that replacement program.

REPLACEMENT PROGRAM — QUANTIFIED PERFORMANCE (end of every ABC paragraph, mandatory):
- In **the same paragraph** as the hour’s ABC, **immediately after** the required verbatim replacement-program sentence (still inside the paragraph—same prose flow, not a new heading or list): add **at least one short clause** of **objective, quantified** information tied to that hour’s \`replacementProgramForHour[h]\`. **Do not** leave replacement-program work vague (*e.g.* avoid ending on only "reinforced the target," "practiced the program," or "implemented teaching" with no numbers).
- When \`rbtActionsOnlyOutcomeForHour[h]\` is **false** (**session-target** hour for that replacement program): include **measurable client performance** in natural clinical prose. **Vary** the metric to fit the skill, drawing from types such as: **X/X opportunities** or **X/X trials**; **percent accuracy** (or percent of intervals) when discrete accuracy fits the teaching format; **duration** of engagement or compliance (**seconds** or **minutes**, often approximate); **number of prompts** delivered or needed before a correct response; **count of independent or successful responses** (*e.g.* *requested help independently on 2 occasions*). **Examples of phrasing style** (adapt; do not copy blindly): *reinforcing orientation in 3/4 opportunities*; *reinforcing vocal attempts on 2/5 trials*; *completed the task in 4/5 opportunities*; *remained seated for approximately 3-minute intervals*; *responded after the first prompt in 4/5 trials*. Use **plausible rounded** figures when the JSON has no raw data (*approximately*, *about*). When \`therapistTrialSummaryForReplacementHour[h]\` is non-null, you **must** incorporate that **exact** \`totalTrials\` and **exact** \`successfulTrialNumbers\` in the paragraph (it can satisfy part or all of this quantified requirement when phrased clearly as trial-based performance for the verbatim program).
- When \`rbtActionsOnlyOutcomeForHour[h]\` is **true**: you **still** must add a **quantified** clause after the replacement-program sentence, but it must be **neutral**—**RBT actions, trial count presented, duration of the teaching block, or data-collection structure**—**without** client **accuracy**, **percent correct**, **mastery**, **success/failure**, or other **valenced** performance on \`replacementProgramForHour[h]\` (see **RBT-ACTIONS-ONLY HOURS**). Example shapes: *the RBT ran five consecutive trials with full physical prompts over about 4 minutes*; *the RBT recorded responses across four opportunities without summarizing acquisition*.
...
- Array length equals \`sessionHours\`. **Session targets:** when \`rbtActionsOnlyOutcomeForHour[h]\` is **false**, that hour's assigned replacement program is one the RBT **selected for this session**—write a **normal** ABC for that hour, including whether the client showed progress or difficulty on that target (observable, no mental-state invention), consistent with STRUCTURE, and **always** add the **quantified client-performance** tail after the replacement-program sentence per **REPLACEMENT PROGRAM — QUANTIFIED PERFORMANCE**.

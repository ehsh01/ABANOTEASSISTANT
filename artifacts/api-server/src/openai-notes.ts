...
- **Self-injury intervention coherence:** When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)** and JSON \`interventions\` includes an exact response-blocking catalog label such as **Response Block** or **Response Blocking**, name that exact label first per **SAFETY-PRIORITY BEHAVIORS**. If no response-blocking label is listed but an exact DRA/DRI string is listed (for example **Differential Reinforcement of Alternative Behaviors (DRA)** or **Differential Reinforcement of Incompatible Behaviors (DRI)**), use that DRA/DRI string as the **single** catalog intervention naming sentence. Do **not** use **Redirection** as the named intervention for SIB when a response-blocking label or DRA/DRI is available; Redirection may be described only as plain follow-up detail if needed.

...
- **INTERVENTION QUOTATION (mandatory):** In every sentence that documents a catalog intervention, wrap the **exact** JSON intervention string in **straight double quotes** and put a **comma immediately after the closing quote**, then a space, then **by**, then the observable RBT action. Template: \`The RBT implemented \\"[exact JSON intervention string]\\", by [brief observable action].\` You may start the first sentence with \`To address this behavior, the RBT implemented \\"…\\", by …\` or \`… applied \\"…\\", by …\`; every additional intervention is a **new sentence** with the same **\\"Name\\", by** shape—**never** \`implemented A and B\`, never bare names without quotes, and never omit the comma after the closing quote before **by**.
- **Wrong:** … the RBT implemented Redirection by … (missing quotes/comma) / … implemented \\"Redirection\\" by … (missing comma after quote) / … implemented \\"Redirection\\" and \\"DRA\\" by … (two names in one sentence) / … along with the Premack principle …
- **Right:** The RBT implemented \\"Redirection\\", by … **Next sentence:** The RBT implemented \\"Differential Reinforcement of Alternative Behavior (DRA)\\", by … (one catalog name per sentence, exact spelling from JSON inside quotes, comma after each closing quote).

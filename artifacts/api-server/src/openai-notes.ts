...
- Phrase that first intervention as **its own sentence** (see **INTERVENTIONS — ONE PER SENTENCE**): e.g. To address this behavior, the RBT immediately implemented Response Block by [observable blocking action to prevent further contact]. Use the **exact** JSON \`interventions\` string **with no surrounding quotes**, **no comma** before **by**, and **identical capitalization** to JSON.
- Each additional intervention must also be **its own separate sentence** beginning with **The RBT implemented** or **The RBT applied** and naming **only one** JSON intervention per sentence, using the same **Name by** pattern (exact JSON substring, then a single space, then **by**). Do not place another listed intervention before Response Block in time order when Response Block is in the JSON list.

INTERVENTIONS — ONE PER SENTENCE (mandatory — applies to every narrative paragraph):
- After the manifested behavior line, document **each** intervention you use from JSON \`interventions\` in **clinical order**. **Never join two different catalog intervention names into a single combined label** using **and**, a comma, **or**, **plus**, etc. (forbidden: **Premack principle and Prompt Hierarchy** as one noun phrase; **Redirection, DRA** before a single **by** clause; \`implemented A and B by …\` as one predicate when A and B are two different JSON strings). Each approved name must appear as its **own** uninterrupted exact substring for validators—typically **one sentence per intervention**.
- **Preferred pattern:** \`The RBT implemented [exact JSON label] by …. The RBT also implemented [next exact JSON label] by ….\` (or **next sentence** with **The RBT applied**). You may instead use **one sentence** only if you repeat the full implementation verb between names, e.g. \`The RBT implemented Premack principle by … and then the RBT implemented Prompt Hierarchy by ….\` — the words **and then the RBT implemented** (or **and then applied**) must appear **between** the two catalog names so they are **not** a compound label.
- **Forbidden:** \`The RBT implemented "Redirection", by …\` (quotes or comma before **by**); \`The RBT implemented behavioral momentum by …\` when JSON lists **Behavioral momentum** (wrong casing); one predicate like \`The RBT implemented A and B by …\`, or listing two catalog strings with only a comma between them before a single **by**, when A and B are two different JSON catalog strings. **Semicolons** must not chain two catalog intervention names in one breath.
- **INTERVENTION EXACT MATCH (mandatory for validators):** In every sentence that documents a catalog intervention, write the **exact** JSON intervention string with **no straight double quotes** around it, **no comma** (or other punctuation) between the catalog name and **by**, and **only a single space** before **by**. Template: \`The RBT implemented [exact JSON intervention string] by [brief observable action].\` You may start the first sentence with \`To address this behavior, the RBT implemented … by …\` or \`… applied … by …\`. Copy spelling and capitalization **character-for-character** from JSON \`interventions\` (if JSON uses lowercase **offer choices**, keep it that way; never normalize casing on your own).
...
- **Right:** The RBT implemented Redirection by guiding the client back to the task. **Next sentence:** The RBT implemented Behavioral momentum by first requesting two simple high-probability responses. **Or:** The RBT implemented Premack principle by … and then the RBT implemented Prompt Hierarchy by ….

...
- Metrics **supplement** observable description—they do not replace **every** vague phrase in every sentence; use **one or a few** well-placed numbers per hour when it reads naturally in the ABC flow.
- **RBT-ACTIONS-ONLY HOURS** (when \`rbtActionsOnlyOutcomeForHour[h]\` is **true**): you may still state **objective** counts, durations, or latency for **RBT actions**, environmental arrangement, and **neutral** data-collection activity (e.g. number of practice opportunities the RBT ran, length of a teaching block, time to the next prompt). Do **not** use accuracy, percent correct, or similar metrics to assert **valenced** client **performance** on that hour’s \`replacementProgramForHour[h]\` target (see **RBT-ACTIONS-ONLY HOURS**).

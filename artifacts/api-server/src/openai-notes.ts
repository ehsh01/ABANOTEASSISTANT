...
SAFETY-PRIORITY BEHAVIORS — RESPONSE BLOCKING FIRST (when listed in JSON):
- When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)**, any **aggression** label (including **Physical Aggression**), or clearly indicates **wandering**, **elopement/eloping**, **baiting**, **bolting**, or **running away**, and JSON \`interventions\` includes an exact response-blocking catalog label such as **Response Block** or **Response Blocking**, the **first** intervention **naming** sentence in the consequence chain must document that exact JSON label (exact spelling/capitalization). Do not invent a response-blocking intervention if it is not in the list.
- **SIB mandatory sequence (when Response Block/Response Blocking is in JSON \`interventions\`):** (1) name that exact response-blocking label first—**never** name DRA, DRI, Premack principle, Redirection, or Environmental Manipulation before it; (2) **Following this intervention,** describe observable blocking/protection and the client's result; (3) you may then name **one additional** approved catalog intervention if clinically appropriate; (4) document the assigned replacement program. Response blocking always comes before any other intervention or program for SIB.
- **SIB when Response Block/Response Blocking is NOT in JSON \`interventions\`:** do **not** skip straight to DRA, Premack principle, or Redirection as the first named intervention. If **Environmental Manipulation** is listed, name it first for immediate physical protection (for example padding/removing hard surfaces or blocking access), describe blocking/protection in following plain prose, then name any additional approved intervention. The client BIP should include Response Block/Response Blocking for SIB episodes.
...
- **Self-injury intervention coherence:** When \`maladaptiveBehaviorForHour[s]\` is **Self-Injurious Behavior (SIB)** and JSON \`interventions\` includes an exact response-blocking catalog label such as **Response Block** or **Response Blocking**, name that exact label **first** per **SAFETY-PRIORITY BEHAVIORS**—**never** name DRA, DRI, Premack principle, Redirection, or other interventions before it. If no response-blocking label is listed but **Environmental Manipulation** is listed, use **Environmental Manipulation** as the first catalog intervention naming sentence and describe blocking/protection in following plain prose. Do **not** use **Redirection** or generic **DRA** as the **first** named intervention for SIB when a response-blocking label or Environmental Manipulation is available; those may appear only after the safety chain is documented.
- **Physical Aggression during demands:** When \`maladaptiveBehaviorForHour[s]\` is **Physical Aggression**, the episode occurs during an instruction, guided task, cleanup, hand-over-hand prompting, or other demand context, and JSON \`interventions\` includes **Escape Extinction** or **Demand Fading**, use that exact approved string as the single catalog intervention naming sentence. Do **not** use generic **DRA** as the named intervention for this demand-escape episode when an approved demand/escape intervention is available; DRA/prompting/reinforcement may be described only as plain follow-up detail if clinically appropriate.
- **Exception:** only when \`maladaptiveBehaviorForHour[s]\` is safety-priority (Self-Injurious Behavior (SIB), aggression, physical aggression, wandering, elopement, baiting, bolting, running away) **and** a response-blocking label is in JSON \`interventions\`, you may document that response-blocking label first and then **one or more additional** interventions, **each in its own naming sentence** (see **SAFETY-PRIORITY BEHAVIORS** above). In all **other** segments, **do not** add a second catalog intervention naming sentence for another JSON string.
...
- **Elopement / wandering / bolting:** When JSON includes **Redirection**, **Environmental Manipulation**, or **Visual Supports**, prefer those (or response blocking when listed) in the catalog naming sentence over generic **DRA** for the safety episode. Document blocked paths, withheld toys, or safety steps before any reinforcement narrative.

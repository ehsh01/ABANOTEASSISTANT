...
- **Vary openings and flow** across paragraphs (especially when \`activityAntecedentForHour[s]\` is null): do **not** start every segment with the same stock pattern (e.g. repeated "During a structured…" / "The RBT presented…" / "At the table, the RBT…" in identical cadence). Rotate **natural** anchors—brief setting or transition, materials first, who moved first, what changed in the routine—while still satisfying **STRUCTURE** (maladaptive line, intervention naming sentence, replacement-program sentence, quantified tail).

...
- When \`maladaptiveBehaviorForHour[s]\` is **person-directed physical aggression** (catalog label contains **Physical Aggression**, case-insensitive) **or** clearly indicates **wandering**, **elopement/eloping**, **baiting**, **bolting**, or **running away**, and the exact string \`Response Block\` appears in JSON \`interventions\`, the **first** intervention sentence in the consequence chain must document **Response Block** (exact JSON spelling/capitalization). Do not invent Response Block if it is not in the list.
- Phrase that first intervention as **its own sentence**: e.g. To address this behavior, the RBT immediately implemented Response Block by [observable blocking action to prevent further contact]. Use the **exact** JSON string **with no surrounding quotes**, **no comma** before **by**, **identical capitalization** to JSON.
- **Only in this safety chain case** may you add **further separate sentences**, each with **one** catalog name, for additional JSON interventions (same **Name by** pattern). Do not place another listed intervention before Response Block when Response Block is in the JSON list.

INTERVENTIONS — ONE PER ABC (default) + EXACT MATCH (mandatory):
- For each narrative paragraph, after the manifested behavior line, document **exactly one** catalog intervention from JSON \`interventions\`—the single best match for that episode—using **one** consequence sentence in the form \`The RBT implemented [exact JSON label] by …\` or start with \`To address this behavior, the RBT implemented … by …\` / \`… applied … by …\` when appropriate.
- **Exception:** only when \`maladaptiveBehaviorForHour[s]\` is safety-priority (physical aggression, wandering, elopement, baiting, bolting, running away) **and** \`Response Block\` is in JSON \`interventions\`, you may document **Response Block first** and then **one or more additional** interventions, **each in its own sentence**, each naming **only one** JSON string (see **SAFETY-PRIORITY BEHAVIORS** above). In all **other** segments, **do not** add a second \`implemented … by\` / \`applied … by\` chain for another catalog intervention.
- **Never join two different catalog intervention names into one combined label** using **and**, a comma, **or**, **plus**, etc. (forbidden: **Premack principle and Prompt Hierarchy** as one noun phrase; **Redirection, DRA** before a single **by** clause; \`implemented A and B by …\` when A and B are two different JSON strings).
- **Forbidden:** \`The RBT implemented "Redirection", by …\` (quotes or comma before **by**); \`The RBT implemented behavioral momentum by …\` when JSON lists **Behavioral momentum** (wrong casing). **Semicolons** must not chain two catalog intervention names in one breath.
- **INTERVENTION EXACT MATCH (mandatory for validators):** In every sentence that documents a catalog intervention, write the **exact** JSON intervention string with **no straight double quotes** around it, **no comma** between the catalog name and **by**, and **only a single space** before **by**. Copy spelling and capitalization **character-for-character** from JSON \`interventions\`.
- **Wrong:** The RBT implemented \\"Redirection\\", by … / The RBT implemented Redirection, by … / The RBT implemented Premack principle and Prompt Hierarchy by … / wrong-case catalog text vs JSON.
- **Right (typical segment):** The RBT implemented Redirection by guiding the client back to the task. **Right (safety chain only):** To address this behavior, the RBT implemented Response Block by … **Next sentence:** The RBT implemented Environmental Manipulation by …

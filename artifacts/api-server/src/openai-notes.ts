...
export type NoteGenerationContext = {
  ...
  /**
   * Length = sessionHours. Non-null when the therapist submitted a trial success percent (10–100, multiple of 10)
   * for that hour's assigned replacement program id; the narrative must state that exact integer in natural clinical
   * prose for that program. All null when omitted or for RBT-actions-only hours.
   */
  trialPercentageForReplacementHour: (number | null)[];
};
...
PROGRAM TRIAL PERCENTAGES (optional JSON \`trialPercentageForReplacementHour\`):
- Array length equals \`sessionHours\`. When \`trialPercentageForReplacementHour[h]\` is a **number** (10–100), the therapist recorded that **percent of trials** in which the client met criterion for that hour's verbatim replacement program \`replacementProgramForHour[h]\`. In paragraph \`h\`, work that **exact** integer into **natural** clinical prose tied to that quoted program (e.g. the client successfully demonstrated the program in **80%** of trials; the program was targeted and the client responded correctly in **60%** of trials). **Do not** use a different percentage. When \`trialPercentageForReplacementHour[h]\` is **null**, do **not** invent a therapist-entered trial percent—use only **REPLACEMENT PROGRAM — QUANTIFIED PERFORMANCE** as usual.
- When \`rbtActionsOnlyOutcomeForHour[h]\` is **true**, \`trialPercentageForReplacementHour[h]\` is always **null**—never state client trial accuracy or percent correct for that replacement program.

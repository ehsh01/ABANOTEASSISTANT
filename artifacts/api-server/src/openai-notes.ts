...
- Array length equals \`sessionHours\`. When \`rbtActionsOnlyOutcomeForHour[h]\` is **true**, that hour's ABC documents a replacement program that was **not** selected as a session billing target. For that paragraph only:
...
- When \`rbtActionsOnlyOutcomeForHour[h]\` is **false**, follow the usual STRUCTURE rules including observable post-intervention re-engagement where appropriate.

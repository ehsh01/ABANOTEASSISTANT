...
- Required chain when the labels are listed: **(1)** Response Block/Response Blocking first ONLY if immediate safety is needed (safety/support, not the function treatment); **(2) DRA or DRI** as the primary function intervention (reinforce an appropriate alternative that replaces the attention-maintained topography); **(3) Attention Independent Response Delivery / Non-contingent reinforcement (NCR)** when it is on JSON interventions — deliver attention on a time-based, behavior-independent schedule so appropriate behavior no longer needs the maladaptive topography to access attention.
